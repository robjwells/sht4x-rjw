...
impl<I: I2c> SHT40<I> {
    ...
    pub fn serial_number(&mut self) -> u32 {
        self.serial_number_with_settings(self.config.validate_crc)
    }

    pub fn serial_number_with_settings(&mut self, validate_crc: bool) -> u32 {
        self.i2c.write(self.address, &[0x89]).unwrap();
        self.i2c.read(self.address, &mut self.read_buffer).unwrap();
    ...
    pub fn measure_with_settings(
        ...
        self.i2c.write(self.address, &[command]).unwrap();
        delay.delay_us(us);
        self.i2c.read(self.address, &mut self.read_buffer).unwrap();
